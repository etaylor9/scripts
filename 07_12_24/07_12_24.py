# -------------------------------- #
# File Info: July 12th, 2024
    # Goals for this file: 
    #     - try a smaller coherence length for f = 1 (maybe go from 600nm --> 50nm??)
    #     - clean up the code and make it more modular
    #     - make the coherence length interable 
# -------------------------------- #
...
def make_video_from_solution(
    ...
    save_path = os.path.join(save_dir, f'solution_video_{f:.2f}_{coherence_length}.mp4')
...
step_x = 200 + width_island # nm
step_y = 200 + width_island # nm
center = (0, 0)
...
plot_save_path = os.path.join(file_path, f'epsilon_distribution_sym.png')
...
if coherence_length < 100:
    IslandDevice.make_mesh(max_edge_length=coherence_length)
else: 
...
mesh_plot_save_path = os.path.join(file_path, f'device_mesh.png')
...
options = tdgl.SolverOptions(
    ...
    current_units="uA",
    field_units="mT",
    output_file=os.path.join(tempdir.name, f"data_{f:.2f}_{coherence_length}.h5"),
)
...
order_parameter_path = os.path.join(file_path, f'order_parameter_sym_{f:.2f}_{coherence_length}.png')
fig , ax = excitation_solution.plot_order_parameter()
fig.savefig(order_parameter_path)


fig, ax = excitation_solution.plot_currents()
current_path = os.path.join(file_path, f'current_sym_{f:.2f}_{coherence_length}.png')
